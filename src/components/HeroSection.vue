...
const logoMotionLayer = ref<HTMLElement | null>(null)
...
onMounted(() => {
  ...
  if (logoMotionLayer.value) {
    gsap.to(logoMotionLayer.value, {
      scrollTrigger: {
        trigger: 'section',
        start: 'top top',
        end: 'bottom top',
  ...
  let logoSkewSetter = gsap.quickSetter(logoMotionLayer.value, "skewY", "deg")
...
<template>
  <section class="relative min-h-screen flex flex-col justify-center items-center overflow-hidden py-20">
    ...
    <div ref="backgroundLogo" class="absolute top-0 bottom-0 left-1/2 w-screen -translate-x-1/2 z-0 pointer-events-none opacity-10 dark:opacity-5 grid place-items-center">
      <div ref="logoMotionLayer" class="hero-logo-frame flex items-center justify-center">
        <svg viewBox="0 0 100 78" fill="none" preserveAspectRatio="xMidYMid meet" class="h-full w-full text-[color:var(--color-logo-faint)]">
